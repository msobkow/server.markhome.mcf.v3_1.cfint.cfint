	...
	implements ICFIntISOTZoneObj
{
	...
	public ICFSecISOTZoneObj realise() {
		ICFSecISOTZoneObj retobj = ((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj().realiseISOTZone(
	...
	@Override
	public void forget() {
		((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj().reallyDeepDisposeISOTZone( (ICFSecISOTZoneObj)this );
	}

	@Override
	public ICFSecISOTZoneObj read() {
		ICFSecISOTZoneObj retobj = ((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj().readISOTZoneByIdIdx( getPKey(), false );
		return( (ICFSecISOTZoneObj)retobj );
	}

	@Override
	public ICFSecISOTZoneObj read( boolean forceRead ) {
		ICFSecISOTZoneObj retobj = ((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj().readISOTZoneByIdIdx( getPKey(), forceRead );
		return( (ICFSecISOTZoneObj)retobj );
	}

	@Override
	public ICFSecISOTZoneTableObj getISOTZoneTable() {
		return( ((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj() );
	}
	...
	public ICFSecISOTZone getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableISOTZone().readDerivedByIdIdx( ((ICFIntSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecISOTZoneEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj().lockISOTZone( getPKey() );
		}
		edit = ((ICFIntSchemaObj)getSchema()).getISOTZoneTableObj().newEditInstance( lockobj );
