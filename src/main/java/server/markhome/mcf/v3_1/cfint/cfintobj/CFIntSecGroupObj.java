	...
	implements ICFIntSecGroupObj
{
	...
	public ICFSecSecGroupObj realise() {
		ICFSecSecGroupObj retobj = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().realiseSecGroup(
	...
	@Override
	public void forget() {
		((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().reallyDeepDisposeSecGroup( (ICFSecSecGroupObj)this );
	}

	@Override
	public ICFSecSecGroupObj read() {
		ICFSecSecGroupObj retobj = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getPKey(), false );
		return( (ICFSecSecGroupObj)retobj );
	}

	@Override
	public ICFSecSecGroupObj read( boolean forceRead ) {
		ICFSecSecGroupObj retobj = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecGroupObj)retobj );
	}

	@Override
	public ICFSecSecGroupTableObj getSecGroupTable() {
		return( ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj() );
	}
	...
	public ICFSecSecGroup getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecGroup().readDerivedByIdIdx( ((ICFIntSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecGroupEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().lockSecGroup( getPKey() );
		}
		edit = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( ( requiredContainerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerCluster = ((ICFIntSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getSecGroupRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public List<ICFSecSecGrpIncObj> getOptionalComponentsInclude() {
		...
		retval = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecSecGrpMembObj> getOptionalComponentsMember() {
		...
		retval = ((ICFIntSchemaObj)getSchema()).getSecGrpMembTableObj().readSecGrpMembByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecSecGrpIncObj> getRequiredChildrenIncByGroup() {
		List<ICFSecSecGrpIncObj> retval;
		retval = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByIncludeIdx( getPKey(),
			false );
		return( retval );
	}
