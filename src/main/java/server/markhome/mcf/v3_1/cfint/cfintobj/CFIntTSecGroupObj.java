	...
	implements ICFIntTSecGroupObj
{
	...
	public ICFSecTSecGroupObj realise() {
		ICFSecTSecGroupObj retobj = ((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj().realiseTSecGroup(
	...
	@Override
	public void forget() {
		((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj().reallyDeepDisposeTSecGroup( (ICFSecTSecGroupObj)this );
	}

	@Override
	public ICFSecTSecGroupObj read() {
		ICFSecTSecGroupObj retobj = ((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getPKey(), false );
		return( (ICFSecTSecGroupObj)retobj );
	}

	@Override
	public ICFSecTSecGroupObj read( boolean forceRead ) {
		ICFSecTSecGroupObj retobj = ((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj().readTSecGroupByIdIdx( getPKey(), forceRead );
		return( (ICFSecTSecGroupObj)retobj );
	}

	@Override
	public ICFSecTSecGroupTableObj getTSecGroupTable() {
		return( ((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj() );
	}
	...
	public ICFSecTSecGroup getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableTSecGroup().readDerivedByIdIdx( ((ICFIntSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecTSecGroupEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj().lockTSecGroup( getPKey() );
		}
		edit = ((ICFIntSchemaObj)getSchema()).getTSecGroupTableObj().newEditInstance( lockobj );
	...
	public ICFSecTenantObj getRequiredContainerTenant( boolean forceRead ) {
		if( ( requiredContainerTenant == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerTenant = ((ICFIntSchemaObj)getSchema()).getTenantTableObj().readTenantByIdIdx( getTSecGroupRec().getRequiredTenantId(), forceRead );
			}
		}
	...
	public List<ICFSecTSecGrpIncObj> getOptionalComponentsInclude() {
		...
		retval = ((ICFIntSchemaObj)getSchema()).getTSecGrpIncTableObj().readTSecGrpIncByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecTSecGrpMembObj> getOptionalComponentsMember() {
		...
		retval = ((ICFIntSchemaObj)getSchema()).getTSecGrpMembTableObj().readTSecGrpMembByGroupIdx( getPKey(),
			false );
		return( retval );
	}
	...
	public List<ICFSecTSecGrpIncObj> getRequiredChildrenIncByGroup() {
		List<ICFSecTSecGrpIncObj> retval;
		retval = ((ICFIntSchemaObj)getSchema()).getTSecGrpIncTableObj().readTSecGrpIncByIncludeIdx( getPKey(),
			false );
		return( retval );
	}
