	...
	implements ICFIntHostNodeObj
{
	...
	public ICFSecHostNodeObj realise() {
		ICFSecHostNodeObj retobj = ((ICFIntSchemaObj)getSchema()).getHostNodeTableObj().realiseHostNode(
	...
	@Override
	public void forget() {
		((ICFIntSchemaObj)getSchema()).getHostNodeTableObj().reallyDeepDisposeHostNode( (ICFSecHostNodeObj)this );
	}

	@Override
	public ICFSecHostNodeObj read() {
		ICFSecHostNodeObj retobj = ((ICFIntSchemaObj)getSchema()).getHostNodeTableObj().readHostNodeByIdIdx( getPKey(), false );
		return( (ICFSecHostNodeObj)retobj );
	}

	@Override
	public ICFSecHostNodeObj read( boolean forceRead ) {
		ICFSecHostNodeObj retobj = ((ICFIntSchemaObj)getSchema()).getHostNodeTableObj().readHostNodeByIdIdx( getPKey(), forceRead );
		return( (ICFSecHostNodeObj)retobj );
	}

	@Override
	public ICFSecHostNodeTableObj getHostNodeTable() {
		return( ((ICFIntSchemaObj)getSchema()).getHostNodeTableObj() );
	}
	...
	public ICFSecHostNode getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableHostNode().readDerivedByIdIdx( ((ICFIntSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecHostNodeEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFIntSchemaObj)getSchema()).getHostNodeTableObj().lockHostNode( getPKey() );
		}
		edit = ((ICFIntSchemaObj)getSchema()).getHostNodeTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredContainerCluster( boolean forceRead ) {
		if( ( requiredContainerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredContainerCluster = ((ICFIntSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getHostNodeRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public List<ICFSecServiceObj> getOptionalComponentsService() {
		...
		retval = ((ICFIntSchemaObj)getSchema()).getServiceTableObj().readServiceByHostIdx( getPKey(),
			false );
		return( retval );
	}
