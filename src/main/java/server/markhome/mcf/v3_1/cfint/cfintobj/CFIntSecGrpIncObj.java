	...
	implements ICFIntSecGrpIncObj
{
	...
	public ICFSecSecGrpIncObj realise() {
		ICFSecSecGrpIncObj retobj = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().realiseSecGrpInc(
	...
	@Override
	public void forget() {
		((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().reallyDeepDisposeSecGrpInc( (ICFSecSecGrpIncObj)this );
	}

	@Override
	public ICFSecSecGrpIncObj read() {
		ICFSecSecGrpIncObj retobj = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByIdIdx( getPKey(), false );
		return( (ICFSecSecGrpIncObj)retobj );
	}

	@Override
	public ICFSecSecGrpIncObj read( boolean forceRead ) {
		ICFSecSecGrpIncObj retobj = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().readSecGrpIncByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecGrpIncObj)retobj );
	}

	@Override
	public ICFSecSecGrpIncTableObj getSecGrpIncTable() {
		return( ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj() );
	}
	...
	public ICFSecSecGrpInc getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecGrpInc().readDerivedByIdIdx( ((ICFIntSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecGrpIncEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().lockSecGrpInc( getPKey() );
		}
		edit = ((ICFIntSchemaObj)getSchema()).getSecGrpIncTableObj().newEditInstance( lockobj );
	...
	public ICFSecClusterObj getRequiredOwnerCluster( boolean forceRead ) {
		if( ( requiredOwnerCluster == null ) || forceRead ) {
			...
			if( ! anyMissing ) {
				requiredOwnerCluster = ((ICFIntSchemaObj)getSchema()).getClusterTableObj().readClusterByIdIdx( getSecGrpIncRec().getRequiredClusterId(), forceRead );
			}
		}
	...
	public ICFSecSecGroupObj getRequiredContainerGroup( boolean forceRead ) {
		if( ( requiredContainerGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredContainerGroup = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getSecGrpIncRec().getRequiredSecGroupId(), forceRead );
			}
		}
	...
	public ICFSecSecGroupObj getRequiredParentSubGroup( boolean forceRead ) {
		if( ( requiredParentSubGroup == null ) || forceRead ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				requiredParentSubGroup = ((ICFIntSchemaObj)getSchema()).getSecGroupTableObj().readSecGroupByIdIdx( getSecGrpIncRec().getRequiredIncludeGroupId(), forceRead );
			}
		}
